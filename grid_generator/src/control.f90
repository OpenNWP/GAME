...
program control
  ...
  integer,  allocatable :: to_index(:),from_index(:),trsk_indices(:),trsk_modified_curl_indices(:),adjacent_vector_indices_h(:), &
                           ...
                           is_land(:)
  real(wp), allocatable :: x_unity(:),y_unity(:),z_unity(:),latitude_scalar(:),longitude_scalar(:),z_scalar(:),gravity_potential(:), &
                           z_vector(:),normal_distance(:),latitude_vector(:),longitude_vector(:),direction(:),volume(:), &
                           area(:),trsk_weights(:),latitude_scalar_dual(:),longitude_scalar_dual(:),z_scalar_dual(:),z_vector_dual(:), &
                           normal_distance_dual(:),direction_dual(:),area_dual(:),f_vec(:),triangle_face_unit_sphere(:), &
                           pent_hex_face_unity_sphere(:),rel_on_line_dual(:),inner_product_weights(:),density_to_rhombi_weights(:), &
  ...
  write(*,*) "Output will be written to file %s.\n",output_file)
  allocate(latitude_ico(12))
  allocate(longitude_ico(12))
