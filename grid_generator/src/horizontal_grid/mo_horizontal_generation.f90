...
module mo_horizontal_generation
  ...
  subroutine generate_horizontal_generators(double latitude_ico(),double longitude_ico(),double latitude_scalar(),double longitude_scalar(),double x_unity(),double y_unity(),double z_unity(),int face_edges_reverse(,3),int face_edges(,3),int face_vertices(,3))
    ...
    integer  :: base_index_down_triangles,base_index_old,test_index,last_triangle_bool,old_triangle_on_line_index, &
    ...
    do (int i = 0 i < N_BASIC_TRIANGLES ++i)
      do (int j = 0 j < res_id ++j)
        no_of_triangles_per_face = pow(4,j)
        do (int k = 0 k < no_of_triangles_per_face ++k)
          if (j==0) then
            ...
            call find_triangle_edge_points_from_dual_scalar_on_face_index(dual_scalar_on_face_index,i,j + 1,point_0,point_1,point_2,face_vertices,face_edges,face_edges_reverse)
  ...
  int face_edges_reverse(,3),int face_vertices(,3))
    ...
    enddo
    double triangle_sum_unit_sphere = 0._wp
    double triangle_avg_unit_sphere_ideal = 4._wp*M_PI/N_TRIANGLES
