...
for day_index in range(len(minima)):
	minima[day_index] = np.min(fetch_model_output(output_dir + "/" + run_id + "+" + str(int(day_index*small_earth_rescale*1440)) + "min_surface.nc", "mslp"))
...
plt.grid()
plt.savefig(output_dir + "/" + run_id + "_" + "mslp_min.png")
plt.close("all")
