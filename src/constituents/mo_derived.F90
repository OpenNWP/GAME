...
module mo_derived
  ...
  use mo_constants,        only: m_d,n_a,k_b,M_PI,t_0,r_v,c_d_v,c_v_v,r_d,m_v,gravity,rho_h2o
  ...
  function v_fall_liquid(state,diag,grid,radius,ji,jl)
    ...
    if (jl==1) then
      gravity_local = grid%gravity_m_v(ji,jl+1)
    elseif (jl==n_layers) then
      gravity_local = grid%gravity_m_v(ji,jl)
    else
      gravity_local &
      = grid%inner_product_weights(7,ji,jl)*grid%gravity_m_v(ji,jl) &
      + grid%inner_product_weights(8,ji,jl)*grid%gravity_m_v(ji,jl+1)
    endif
    
    kinematic_viscosity = calc_diff_coeff(diag%temperature(ji,jl),state%rho(ji,jl,n_condensed_constituents+1))
    
    v_fall_liquid = 2._wp*radius**2*(rho_h2o-state%rho(ji,jl,n_condensed_constituents+1))*gravity_local &
    ...
    if (reynolds_number>reynolds_crit) then
      c_w = .8_wp
      v_fall_liquid = sqrt(8._wp*radius*rho_h2o*gravity_local/(3._wp*state%rho(ji,jl,n_condensed_constituents+1)*c_w))
    endif
