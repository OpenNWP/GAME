  ...
  subroutine inner_product(in_field_1,in_field_2,out_field,grid)
    ...
    do h_index=1,n_cells
      ...
      do layer_index=0,n_layers-1
        ...
        base_index = 8*(ji-1)
        out_field(h_index,layer_index+1) = 0._wp
        ...
        enddo
        out_field(h_index,layer_index+1) = out_field(h_index,layer_index+1) + grid%inner_product_weights(h_index,layer_index+1,7) &
        *in_field_1(h_index+layer_index*n_vectors_per_layer) &
        *in_field_2(h_index+layer_index*n_vectors_per_layer)
        out_field(h_index,layer_index+1) = out_field(h_index,layer_index+1) + grid%inner_product_weights(h_index,layer_index+1,8) &
