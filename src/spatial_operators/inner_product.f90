...
module mo_inner_product
  ...
  bind(c,name = "inner_product")
    ...
    real(wp), intent(in)  :: in_field_1(n_vectors),in_field_2(n_vectors),inner_product_weights(8*n_scalars)
    ...
    do h_index=1,n_scalars_h
      ...
      do layer_index=0,n_layers-1
        ...
        out_field(ji) = 0._wp
        do jk=1,no_of_edges
          out_field(ji) = out_field(ji) + inner_product_weights(base_index + jk) &
          *in_field_1(n_scalars_h + layer_index*n_vectors_per_layer + 1 + adjacent_vector_indices_h(6*(h_index-1) + jk)) &
          *in_field_2(n_scalars_h + layer_index*n_vectors_per_layer + 1 + adjacent_vector_indices_h(6*(h_index-1) + jk))
        enddo
        out_field(ji) = out_field(ji) + inner_product_weights(base_index+7) &
        *in_field_1(h_index+layer_index*n_vectors_per_layer) &
        *in_field_2(h_index+layer_index*n_vectors_per_layer)
        out_field(ji) = out_field(ji) + inner_product_weights(base_index+8) &
