...
module mo_write_output
  ...
  subroutine write_out()
    ...
    if (config_io -> surface_output_switch == 1) then
      ...
      call nc_check(nc_def_dim(ncid,"single_int_index",1,single_int_dimid))
      call nc_check(nc_def_dim(ncid,"lat_index",N_LAT_IO_POINTS,lat_dimid))
      call nc_check(nc_def_dim(ncid,"lon_index",N_LON_IO_POINTS,lon_dimid))
      ...
      call nc_check(nc_def_var(ncid,"start_day",NC_INT,1,single_int_dimid,start_day_id))
      call nc_check(nc_def_var(ncid,"start_hour",NC_INT,1,single_int_dimid,start_hour_id))
      call nc_check(nc_def_var(ncid,"lat",NC_DOUBLE,1,lat_dimid,lat_id))
      call nc_check(nc_def_var(ncid,"lon",NC_DOUBLE,1,lon_dimid,lon_id))
      call nc_check(nc_def_var(ncid,"mslp",NC_DOUBLE,2,lat_lon_dimids,mslp_id))
      call nc_check(nc_put_att_text(ncid,mslp_id,"units",strlen("Pa"),"Pa"))
      call nc_check(nc_def_var(ncid,"sp",NC_DOUBLE,2,lat_lon_dimids,sp_id))
      call nc_check(nc_put_att_text(ncid,sp_id,"units",strlen("Pa"),"Pa"))
      call nc_check(nc_def_var(ncid,"t2",NC_DOUBLE,2,lat_lon_dimids,t2_id))
      call nc_check(nc_put_att_text(ncid,t2_id,"units",strlen("K"),"K"))
      call nc_check(nc_def_var(ncid,"tcc",NC_DOUBLE,2,lat_lon_dimids,tcc_id))
      call nc_check(nc_put_att_text(ncid,tcc_id,"units",strlen("%"),"%"))
      call nc_check(nc_def_var(ncid,"rprate",NC_DOUBLE,2,lat_lon_dimids,rprate_id))
      call nc_check(nc_put_att_text(ncid,rprate_id,"units",strlen("kg/(m^2s)"),"kg/(m^2s)"))
      call nc_check(nc_def_var(ncid,"sprate",NC_DOUBLE,2,lat_lon_dimids,sprate_id))
      call nc_check(nc_put_att_text(ncid,sprate_id,"units",strlen("kg/(m^2s)"),"kg/(m^2s)"))
      call nc_check(nc_def_var(ncid,"cape",NC_DOUBLE,2,lat_lon_dimids,cape_id))
      call nc_check(nc_put_att_text(ncid,cape_id,"units",strlen("J/kg"),"J/kg"))
      call nc_check(nc_def_var(ncid,"sfc_sw_down",NC_DOUBLE,2,lat_lon_dimids,sfc_sw_down_id))
      call nc_check(nc_put_att_text(ncid,sfc_sw_down_id,"units",strlen("W/m^2"),"W/m^2"))
      call nc_check(nc_def_var(ncid,"u10",NC_DOUBLE,2,lat_lon_dimids,u10_id))
      call nc_check(nc_put_att_text(ncid,u10_id,"units",strlen("m/s"),"m/s"))
      call nc_check(nc_def_var(ncid,"v10",NC_DOUBLE,2,lat_lon_dimids,v10_id))
      call nc_check(nc_put_att_text(ncid,v10_id,"units",strlen("m/s"),"m/s"))
      call nc_check(nc_def_var(ncid,"gusts10",NC_DOUBLE,2,lat_lon_dimids,gusts_id))
      call nc_check(nc_put_att_text(ncid,gusts_id,"units",strlen("m/s"),"m/s"))
      call nc_check(nc_enddef(ncid))
      
      ! writing the variables
      call nc_check(nc_put_var_int(ncid,start_day_id,init_date))
      call nc_check(nc_put_var_int(ncid,start_hour_id,init_time))
      call nc_check(nc_put_var_double(ncid,lat_id,lat_vector))
      call nc_check(nc_put_var_double(ncid,lon_id,lon_vector))
      call interpolate_to_ll(mslp,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,mslp_id,lat_lon_output_field))
      call interpolate_to_ll(sp,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,sp_id,lat_lon_output_field))
      call interpolate_to_ll(t2,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,t2_id,lat_lon_output_field))
      call interpolate_to_ll(tcc,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,tcc_id,lat_lon_output_field)
      call interpolate_to_ll(rprate,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,rprate_id,lat_lon_output_field))
      call interpolate_to_ll(sprate,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,sprate_id,lat_lon_output_field))
      call interpolate_to_ll(cape,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,cape_id,lat_lon_output_field))
      call interpolate_to_ll(sfc_sw_down,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,sfc_sw_down_id,lat_lon_output_field))
      call interpolate_to_ll(wind_10_m_mean_u_at_cell,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,u10_id,lat_lon_output_field))
      call interpolate_to_ll(wind_10_m_mean_v_at_cell,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,v10_id,lat_lon_output_field))
      call interpolate_to_ll(wind_10_m_gusts_speed_at_cell,lat_lon_output_field,grid)
      call nc_check(nc_put_var_double(ncid,gusts_id,lat_lon_output_field))
    ...
    if (config_io -> pressure_level_output_switch == 1) then
      ...
      call nc_check(nc_create(OUTPUT_FILE_PRESSURE_LEVEL,NC_CLOBBER,ncid))
      call nc_check(nc_def_dim(ncid,"single_int_index",1,single_int_dimid))
      call nc_check(nc_def_dim(ncid,"lat_index",N_LAT_IO_POINTS,lat_dimid))
      call nc_check(nc_def_dim(ncid,"lon_index",N_LON_IO_POINTS,lon_dimid))
      ...
      do (int i = 0 i < N_PRESSURE_LEVELS ++i)
        ...
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,gh_ids(i)))
        call nc_check(nc_put_att_text(ncid,gh_ids(i),"units",strlen("gpm"),"gpm"))
        sprintf(varname,"temperature_layer_%d",pressure_level_hpa)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,temp_p_ids(i)))
        call nc_check(nc_put_att_text(ncid,temp_p_ids(i),"units",strlen("K"),"K"))
        sprintf(varname,"rel_hum_layer_%d",pressure_level_hpa)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,rh_p_ids(i)))
        call nc_check(nc_put_att_text(ncid,rh_p_ids(i),"units",strlen("%"),"%"))
        sprintf(varname,"wind_u_layer_%d",pressure_level_hpa)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,wind_u_p_ids(i)))
        call nc_check(nc_put_att_text(ncid,wind_u_p_ids(i),"units",strlen("m/s"),"m/s"))
        sprintf(varname,"wind_v_layer_%d",pressure_level_hpa)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,wind_v_p_ids(i)))
        call nc_check(nc_put_att_text(ncid,wind_v_p_ids(i),"units",strlen("m/s"),"m/s"))
        sprintf(varname,"rel_vort_layer_%d",pressure_level_hpa)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,epv_p_ids(i)))
        call nc_check(nc_put_att_text(ncid,epv_p_ids(i),"units",strlen("PVU"),"PVU"))
        sprintf(varname,"epv_layer_%d",pressure_level_hpa)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,rel_vort_p_ids(i)))
        call nc_check(nc_put_att_text(ncid,rel_vort_p_ids(i),"units",strlen("K*m^2/(ks*s)"),"K*m^2/(ks*s)"))
      enddo
      ...
      call nc_check(nc_put_var_int(ncid,start_day_id,init_date))
      call nc_check(nc_put_var_int(ncid,start_hour_id,init_time))
      call nc_check(nc_put_var_double(ncid,lat_id,lat_vector(0)))
      call nc_check(nc_put_var_double(ncid,lon_id,lon_vector(0)))
      do (int i = 0 i < N_PRESSURE_LEVELS ++i)
        
        call interpolate_to_ll(geopotential_height(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,gh_ids(i),lat_lon_output_field(0)(0)))
        call interpolate_to_ll(t_on_pressure_levels(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,temp_p_ids(i),lat_lon_output_field(0)(0)))
        call interpolate_to_ll(rh_on_pressure_levels(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,rh_p_ids(i),lat_lon_output_field(0)(0)))
        call interpolate_to_ll(u_on_pressure_levels(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,wind_u_p_ids(i),lat_lon_output_field(0)(0)))
        call interpolate_to_ll(v_on_pressure_levels(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,wind_v_p_ids(i),lat_lon_output_field(0)(0)))
        call interpolate_to_ll(epv_on_pressure_levels(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,epv_p_ids(i),lat_lon_output_field(0)(0)))
        call interpolate_to_ll(rel_vort_on_pressure_levels(i)(0),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,rel_vort_p_ids(i),lat_lon_output_field(0)(0)))
    ...
    if (config_io -> model_level_output_switch == 1) then
      ...
      do (int i = 0 i < n_layers ++i)
        ...
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,temperature_ids(i)))
        call nc_check(nc_put_att_text(ncid,temperature_ids(i),"units",strlen("K"),"K"))
        sprintf(varname,"pressure_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,pressure_ids(i)))
        call nc_check(nc_put_att_text(ncid,temperature_ids(i),"units",strlen("Pa"),"Pa"))
        sprintf(varname,"rel_hum_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,rel_hum_ids(i)))
        call nc_check(nc_put_att_text(ncid,temperature_ids(i),"units",strlen("%"),"%"))
        sprintf(varname,"wind_u_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,wind_u_ids(i)))
        call nc_check(nc_put_att_text(ncid,wind_u_ids(i),"units",strlen("m/s"),"m/s"))
        sprintf(varname,"wind_v_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,wind_v_ids(i)))
        call nc_check(nc_put_att_text(ncid,wind_v_ids(i),"units",strlen("m/s"),"m/s"))
        sprintf(varname,"rel_vort_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,rel_vort_ids(i)))
        call nc_check(nc_put_att_text(ncid,rel_vort_ids(i),"units",strlen("1/s"),"1/s"))
        sprintf(varname,"div_h_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,div_h_ids(i)))
        call nc_check(nc_put_att_text(ncid,div_h_ids(i),"units",strlen("1/s"),"1/s"))
      enddo
      do (int i = 0 i < N_LEVELS ++i)
        sprintf(varname,"wind_w_layer_%d",i)
        call nc_check(nc_def_var(ncid,varname,NC_DOUBLE,2,lat_lon_dimids,wind_w_ids(i)))
        call nc_check(nc_put_att_text(ncid,wind_w_ids(i),"units",strlen("m/s"),"m/s"))
      enddo
      call nc_check(nc_enddef(ncid))
      ...
      do (int i = 0 i < n_layers ++i)
        ...
        call nc_check(nc_put_var_double(ncid,temperature_ids(i),lat_lon_output_field(0)(0)))
        interpolate_to_ll((*pressure)(i*n_scalars_h),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,pressure_ids(i),lat_lon_output_field(0)(0)))
        interpolate_to_ll((*rh)(i*n_scalars_h),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,rel_hum_ids(i),lat_lon_output_field(0)(0)))
        interpolate_to_ll(diagnostics-> u_at_cell(i*n_scalars_h),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,wind_u_ids(i),lat_lon_output_field(0)(0)))
        interpolate_to_ll(diagnostics-> v_at_cell(i*n_scalars_h),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,wind_v_ids(i),lat_lon_output_field(0)(0)))
        interpolate_to_ll((*rel_vort)(i*n_scalars_h),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,rel_vort_ids(i),lat_lon_output_field(0)(0)))
        interpolate_to_ll((*div_h_all_layers)(i*n_scalars_h),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,div_h_ids(i),lat_lon_output_field(0)(0)))
      enddo
      
      do (int i = 0 i < N_LEVELS ++i)
        interpolate_to_ll(state_write_out -> wind(i*n_vectors_per_layer),lat_lon_output_field,grid)
        call nc_check(nc_put_var_double(ncid,wind_w_ids(i),lat_lon_output_field(0)(0)))
      enddo
    ...
    .and. time_since_init_min == time_to_next_analysis_min) then
      ...
      call nc_check(nc_def_dim(ncid,"densities_index",N_CONSTITUENTS*N_SCALARS,densities_dimid))
      ...
      call nc_check(nc_def_var(ncid,"t_soil",NC_DOUBLE,1,soil_dimid,soil_id))
      call nc_check(nc_put_att_text(ncid,soil_id,"units",strlen("K"),"K"))
      call nc_check(nc_enddef(ncid))
      
      ! setting the variables
      call nc_check(nc_put_var_int(ncid,start_day_id,init_date))
      call nc_check(nc_put_var_int(ncid,start_hour_id,init_time))
      call nc_check(nc_put_var_double(ncid,densities_id,state_write_out -> rho(0)))
      call nc_check(nc_put_var_double(ncid,temperature_id,diagnostics -> temperature(0)))
      call nc_check(nc_put_var_double(ncid,wind_id,state_write_out -> wind(0)))
      call nc_check(nc_put_var_double(ncid,tke_id,diagnostics -> tke(0)))
      call nc_check(nc_put_var_double(ncid,soil_id,state_write_out -> temperature_soil(0)))
