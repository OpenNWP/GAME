...
module mo_write_output
  ...
  function pseudopotential_temperature(State *state,Diagnostics *diagnostics,Grid *grid,int scalar_index)
    ...
    if (MOISTURE_ON==0) then
      result = theta_v_bg(scalar_index) + theta_v_pert(scalar_index)
    endif
  ...
  subroutine write_out()
    ...
    if (surface_output_switch==1) then
      ...
      do (int i = 0 i<n_scalars_h ++i)
        ...
        *temp_lowest_layer
        ...
        if (prog_soil_temp==1) then
          temperature_gradient = (temp_closest - temperature_soil(i))
        ...
        do while (z_height<z_tropopause)
          ...
          if (cape_integrand>0._wp) then
            cape(i) = cape(i) + cape_integrand*delta_z
          endif
          layer_index = layer_index-1
          z_height = z_scalar(layer_index*n_scalars_h + i)
        enddo
        
        sfc_sw_down(i) = sfc_sw_in(i)/(1._wp - sfc_albedo(i) + EPSILON_SECURITY)
      ...
      do (int h_index = 0 h_index<N_VECS_H ++h_index)
        ...
        do (int time_step_10_m_wind = 0 time_step_10_m_wind<min_no_of_output_steps ++time_step_10_m_wind)
          ...
          do (int i = 0 i<10 ++i)
            wind_tangential = wind_tangential + trsk_weights(10*h_index + i)*wind_h_lowest_layer_array(time_step_10_m_wind*N_VECS_H + trsk_indices(10*h_index + i))
          enddo
      ...
      do (int i = 0 i<N_VECS_H ++i)
        actual_roughness_length = 0.5_wp*(roughness_length(from_index(i)) + roughness_length(to_index(i)))
        ! roughness length of grass according to WMO
        roughness_length_extrapolation = 0.02_wp
        if (is_land(from_index(i))==0) then
          roughness_length_extrapolation = actual_roughness_length
        endif
        z_sfc = 0.5_wp*(z_vector(n_vectors - n_scalars_h + from_index(i)) + z_vector(n_vectors - n_scalars_h + to_index(i)))
    ...
    div_h(wind,*div_h_all_layers,
    adjacent_signs_h,adjacent_vector_indices_h,inner_product_weights,slope,area,volume)
    calc_rel_vort(wind,rel_vort_on_triangles,z_vector,z_vector_dual,rel_vort,
    ...
    curl_field_to_cells(rel_vort,*rel_vort,adjacent_vector_indices_h,inner_product_weights)
    
    ! Diagnozing the u and v wind components at the vector points.
    calc_uv_at_edge(wind,u_at_edge,v_at_edge,trsk_indices,trsk_weights,direction)
    ! Averaging to cell centers for output.
    edges_to_cells(u_at_edge,u_at_cell,adjacent_vector_indices_h,inner_product_weights)
    edges_to_cells(v_at_edge,v_at_cell,adjacent_vector_indices_h,inner_product_weights)
    ...
    epv_diagnostics(epv,from_index,to_index,inner_product_weights,pot_vort,trsk_indices,trsk_weights,
