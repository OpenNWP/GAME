...
module mo_scalar_tend_expl
  ...
  subroutine scalar_tend_expl(state_old,state_new,state_tend,diag,grid,rk_step)
    ...
    if (ltemp_diff_h .and. rk_step==1) then
      ...
      call scalar_times_vector_h(diag%temp_diff_coeff_eff_h,diag%vector_placeholder_h,diag%flux_density_h,grid)
      ...
      if (ltemp_diff_v) then
        call scalar_times_vector_v(diag%temp_diff_coeff_eff_v,diag%vector_placeholder_v,diag%flux_density_v)
    ...
    if (lmass_diff_h .and. rk_step==1) then
      ...
      do jc=1,n_constituents
        ...
        call scalar_times_vector_h(diag%mass_diff_coeff_eff_h,diag%vector_placeholder_h,diag%flux_density_h,grid)
        ...
        if (lmass_diff_v) then
          call scalar_times_vector_v(diag%mass_diff_coeff_eff_v,diag%vector_placeholder_v,diag%flux_density_v)
