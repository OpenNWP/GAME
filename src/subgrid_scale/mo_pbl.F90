...
module mo_pbl
  ...
  subroutine update_sfc_turb_quantities(state,diag,grid)
    ...
    do ji=1,n_cells
      ...
      u10 = u_lowest_layer*log(10._wp/diag%roughness_length(ji))/log(z_agl/diag%roughness_length(ji))
      ...
      if (grid%land_fraction(ji)<1._wp) then
        ...
        diag%roughness_length(ji) = grid%land_fraction(ji)*grid%roughness_length(ji) &
                                    + (1._wp - grid%land_fraction(ji))*roughness_length_from_u10_sea(u10)
      endif
      
      ! updating the roughness velocity
      diag%roughness_velocity(ji) = calc_roughness_velocity(u_lowest_layer,z_agl,diag%roughness_length(ji))
